...
SRE 업무를 수행하면서 실행 시간이 200ms 를 초과하는 쿼리들을 대상으로 최적화 작업을 진행하게 되었습니다. 그 중 PostgreSQL 의 `UPDATE CASE WHEN` 문과 인덱스를 활용해 쿼리를 개선해본 경험을 공유하려 합니다.
...
```
<br />
결과적으로, 두 번 수행되던 UPDATE 쿼리를 한 번으로 통합하고 평균 실행 시간을 259ms에서 3.24ms로 단축함으로써, 총 실행 시간을 `518ms → 3.24ms`로 개선할 수 있었습니다.