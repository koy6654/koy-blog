...
그 중 인덱스가 존재하고 조건을 충족함에도 PostgreSQL 이 자체적으로 판단하여 인덱스 스캔을 타지 않아 쿼리 성능에 문제가 발생하는  건을 개선해본 경험을 공유해보려 합니다.

---

## PostgreSQL이 인덱스를 사용하지 않는 경우

쿼리를 인덱스 조건에 맞춰도 아래와 같은 경우 PostgreSQL 예외적으로 판단하여 인덱스 스캔을 사용하지 않습니다.
- **테이블 크기가 작을 경우**<br />
...
Limit (cost=0.00..8.47 rows=10 width=54) (actual time=665.355..665.359 rows=10 loops=1)
   ->  Seq Scan on test (cost=0.00..199438.52 rows=235499 width=54) (actual time=665.354..665.357 rows=10 loops=1)
         Filter: ((name = 'new_name'::text) AND (status = 'status'::text) AND (field = 'field'::text) AND (hash = 'hash'::text) AND (menu = 'menu'::text))
...
***(참고)<br />
Index Scan + Heap Fetch 조합 vs Seq Scan 에서 Seq Scan이 더 빠를 수 있는 이유
...
CREATE INDEX test_index_only_scan_idx 
ON test (name, status, field, hash, menu, id);
...
Index Only Scan using test_index_only_scan_idx on test (cost=0.43..225.32 rows=10 width=4) (actual time=0.025..0.074 rows=10 loops=1)
  Index Cond: ((name = 'name'::text) AND (status = 'status') AND (field = 'field'::text) AND (hash = 'hash'::text) AND (menu = 'menu'::text))
  Heap Fetches: 0
Planning Time: 0.110 ms
Execution Time: 0.103 ms
```

효과는 극적이였지만, 동일한 조건의 쿼리 하나만을 위해 단순히 컬럼 하나를 추가한 중복 인덱스를 생성하는 것은 디스크 용량 증가나 쓰기 성능 저하 등의 부담이 크다고 판단했습니다.
따라서 `ORDER BY name, status` 절을 활용해 인덱스와 정렬 방향을 일치시키면, PSQL 옵티마이저가 기존 인덱스를 활용하도록 유도할 수 있지 않을까 하는 생각에 아래와 같이 시도해보게 되었습니다.
...
결과적으로, 실제로 PSQL이 의도한 대로 Index Scan을 수행하는 것을 확인했으며, 다음과 같은 성능 개선 효과를 확인할 수 있었습니다.
