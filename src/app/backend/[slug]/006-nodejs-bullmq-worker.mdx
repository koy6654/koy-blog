...
기존에 사용하던 cron 기반 스케줄링 작업의 불안정성을 해결하고, 외부 서비스로부터 들어오는 RPC 요청을 안정적으로 Queue로 처리하기 위해 여러 오픈소스 솔루션을 검토했습니다. 다양한 메시지 큐가 있었지만, 현재 상황과 비용을 함께 고려했을 때 BullMQ가 가장 적합하다고 판단했고, 이를 기반으로 아키텍처를 구현했습니다. 이번 포스팅에서는 그 과정에서 알게 된 내용을 정리했습니다.
